...
#### `CreateContainerConfigError` on startup (OpenShift)

**Cause:** Pod is trying to run as UID `1001`, which the default `restricted-v2` SCC blocks.
...
#### Pods stuck in `Pending` — `Insufficient cpu` / `Insufficient memory`

**Cause:** Worker nodes are fully allocated by platform services (Ceph/ODF, Prometheus, etc.).

**Fix:** Allow control-plane nodes to schedule workloads:
```bash
...
#### Control Center crashes with OOMKilled or `504 Gateway Timeout`

**Cause:** Control Center runs internal Kafka Streams topologies for every connected cluster and connector. Tracking multiple Connect clusters pushes it over the default memory limit.

**Fix:** Increase the memory limit in the `ControlCenter` CR:
```yaml
podTemplate:
  resources:
    limits:
...
#### `UNKNOWN_TOPIC_OR_PARTITION` in a connector

**Cause:** `auto.create.topics.enable=false` on the brokers (the default in Confluent Platform).

**Fix:** Pre-create the topic before deploying the connector, either with `kafka-topics --create` or a `KafkaTopic` custom resource:
```yaml
apiVersion: platform.confluent.io/v1beta1
kind: KafkaTopic
metadata:
